...
public class MariasCoreRenderer implements ICurioRenderer {
    ...
    public <T extends LivingEntity, M extends EntityModel<T>> void render(ItemStack stack, SlotContext slotContext, PoseStack matrixStack, RenderLayerParent<T, M> renderLayerParent, MultiBufferSource renderTypeBuffer, int light, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        for (int j = 0; j < 2; j++) {
            matrixStack.mulPose(Vector3f.XP.rotationDegrees(j == 0 ? 0 : 180));

            for (int i = 0; i < 2; i++) {
                matrixStack.pushPose();

                if(j == 0) {
                    matrixStack.mulPose(Vector3f.YP.rotationDegrees(i == 0 ? flap : 180 - flap));
                } else matrixStack.mulPose(Vector3f.YP.rotationDegrees(i == 0 ? flap - 180: -flap));


                matrixStack.translate(-0.5, j == 0 ? 0.7 : -1.7, 0);

                matrixStack.scale(1F, -1F, -1F);
                Minecraft.getInstance().getItemRenderer().render(stack, ItemTransforms.TransformType.NONE, false, matrixStack, renderTypeBuffer, 0xF000F0, OverlayTexture.NO_OVERLAY, model);
                matrixStack.popPose();
            }
        }
